"""Role helpers for dashboard governance (owner > admin > member > viewer)."""
...
def require_min_dashboard_role(auth_session: DashboardAuthSession, *, min_role: str) -> None:
    """Raise 403 when the session membership is below ``min_role``."""
...
def require_member_or_above(auth_session: DashboardAuthSession) -> None:
    """Allows member, admin, owner (excludes viewer for mutating routes)."""
...
async def ensure_dashboard_not_viewer(
    ...
    """Block viewers from mutating routes; API-key fallback remains full access."""
