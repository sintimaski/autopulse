...
async def create_magic_link_token(
    *,
    ...
    if allowed_email:
        ...
        # Production must configure an allowlist, domain gate, or OIDC-only access.
        return None
...
async def issue_dashboard_session_for_user(
    *,
    ...
    """Create or refresh a dashboard user, bind optional IdP claims, and set the session cookie."""
    normalized_email = _normalize_email(email)
    now = _now()
