...
def upgrade() -> None:
    ...
    # Alembic creates ``alembic_version.version_num`` as VARCHAR(32) by default, but
    # several revision identifiers in this project exceed 32 chars. Widen it on dialects
    # that enforce VARCHAR length; SQLite ignores the declared length so it is skipped.
