...
def _sync_env_lumonox_key_bundle(raw_api_key: str) -> None:
    """Best-effort local helper: keep `.env.lumonox` in sync after key issue/rotate."""
    path = Path(os.getenv("LUMONOX_ENV_FILE", ".env.lumonox")).expanduser()
    # Standalone: ``/dashboard`` and ``/ingest`` — not under ``/lumonox`` unless client sets base.
    base = (os.getenv("NEXT_PUBLIC_LUMONOX_API_BASE_URL") or "").strip()
    content = (
        "# Lumonox — synced from dashboard API key lifecycle.\n"
        "# Source before static UI build: set -a && source .env.lumonox && set +a\n"
        f"NEXT_PUBLIC_LUMONOX_API_BASE_URL={base}\n"
        f"LUMONOX_API_KEY={raw_api_key}\n"
        f"NEXT_PUBLIC_LUMONOX_API_KEY={raw_api_key}\n"
    )
    try:
...
async def issue_dashboard_api_key(
    ...
    await session.commit()
    _sync_env_lumonox_key_bundle(raw_api_key)
...
async def get_dashboard_onboarding_status(
    ...
    hints: dict[str, str] = {
        ...
        "send_first_event": (
            "Send traffic from your instrumented app so Lumonox can receive events."
        ),
