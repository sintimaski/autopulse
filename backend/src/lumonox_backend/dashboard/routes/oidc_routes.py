"""OIDC login (Auth0, WorkOS, etc.) — complements magic-link auth."""
...
from lumonox_backend.auth.dashboard import (
...
def _verify_oidc_cookie(secret: str, raw: str, *, max_age_seconds: int = 900) -> dict[str, Any]:
    ...
    payload = cast(dict[str, Any], json.loads(base64.urlsafe_b64decode(padded.encode("utf-8"))))
...
async def oidc_login_start(request: Request) -> Response:
    ...
    cookie_payload = {"verifier": verifier, "state": state, "iat": int(time.time())}
...
async def oidc_login_callback(
    ...
    token_json = cast(dict[str, Any], token_response.json())
    ...
    if isinstance(access_token, str) and isinstance(userinfo_endpoint, str) and userinfo_endpoint:
        ...
        if userinfo_response.status_code < 400:
            claims = cast(dict[str, Any], userinfo_response.json())
