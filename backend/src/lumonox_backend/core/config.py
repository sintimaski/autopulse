...
from lumonox_backend.core.config_env import (
...
class Settings:
    ...
    event_plane_shards_path: str = "./.lumonox/events-log"
    event_plane_snapshots_path: str = "./.lumonox/events-duckdb"
    ...
    parquet_export_root: str = "./.lumonox/parquet/events"
    ...
    parquet_object_storage_prefix: str = "lumonox-parquet"
    ...
    parquet_object_storage_restore_root: str = "./.lumonox/parquet/restore"
    ...
    ingest_drop_lumonox_traffic_from_db: bool = True
    ...
    alert_email_file_outbox_dir: str = "./.lumonox/emails"
    ...
    dashboard_oidc_cookie_name: str = "lumonox_oidc_state"
...
def validate_deployment_settings(settings: Settings) -> None:
    ...
    if scheduler_required_for_env(settings) and not settings.jobs_enable_scheduler:
        raise ValueError(
            "JOBS_ENABLE_SCHEDULER must be true when LUMONOX_ENV is staging/production unless "
    ...
    if (
        ...
        raise ValueError(
            "LUMONOX_DUCKDB_SINGLE_WRITER_PROFILE must be true when "
            "LUMONOX_ENV=production uses LUMONOX_EVENT_STORE=duckdb with "
    ...
    if settings.dashboard_auth_magic_link_dev_expose_token:
        raise ValueError(
            "DASHBOARD_AUTH_MAGIC_LINK_DEV_EXPOSE_TOKEN must be false when LUMONOX_ENV=production"
        )
    if settings.dev_scenarios_enabled:
        raise ValueError("DEV_SCENARIOS_ENABLED must be false when LUMONOX_ENV=production")
    if not settings.dashboard_auth_enabled:
        return
    if settings.dashboard_auth_allow_api_key_fallback:
        raise ValueError(
            "DASHBOARD_AUTH_ALLOW_API_KEY_FALLBACK must be false when LUMONOX_ENV=production "
            "and DASHBOARD_AUTH_ENABLED is true"
        )
    if settings.dashboard_auth_session_ttl_minutes < 30:
        raise ValueError(
            "DASHBOARD_AUTH_SESSION_TTL_MINUTES must be >= 30 when LUMONOX_ENV=production "
            "and DASHBOARD_AUTH_ENABLED is true"
        )
    if settings.dashboard_auth_magic_link_ttl_minutes < 5:
        raise ValueError(
            "DASHBOARD_AUTH_MAGIC_LINK_TTL_MINUTES must be >= 5 when LUMONOX_ENV=production "
            "and DASHBOARD_AUTH_ENABLED is true"
        )
    if settings.dashboard_auth_magic_link_ttl_minutes > 30:
        raise ValueError(
            "DASHBOARD_AUTH_MAGIC_LINK_TTL_MINUTES must be <= 30 when LUMONOX_ENV=production "
            "and DASHBOARD_AUTH_ENABLED is true"
        )
    if settings.dashboard_auth_magic_link_base_url:
        ...
        if parsed_magic_link_base.scheme.lower() != "https":
            raise ValueError(
                "DASHBOARD_AUTH_MAGIC_LINK_BASE_URL must use https when LUMONOX_ENV=production "
                "and DASHBOARD_AUTH_ENABLED is true"
            )
    if settings.dashboard_oidc_enabled and settings.dashboard_oidc_redirect_uri:
        ...
        if parsed_oidc_redirect.scheme.lower() != "https":
            raise ValueError(
                "DASHBOARD_OIDC_REDIRECT_URI must use https when LUMONOX_ENV=production "
                "and DASHBOARD_AUTH_ENABLED is true"
            )
    if not settings.ingest_require_https:
        raise ValueError("INGEST_REQUIRE_HTTPS must be true when LUMONOX_ENV=production")
    if not (settings.internal_metrics_bearer_token or "").strip():
        raise ValueError("INTERNAL_METRICS_BEARER_TOKEN must be set when LUMONOX_ENV=production")
    if any(origin.strip() == "*" for origin in settings.cors_allow_origins):
        raise ValueError(
            "CORS_ALLOW_ORIGINS must not include '*' when LUMONOX_ENV=production "
            "and DASHBOARD_AUTH_ENABLED is true"
        )
    ...
    if not settings.dashboard_enforce_origin_for_mutations:
        raise ValueError(
            "DASHBOARD_ENFORCE_ORIGIN_FOR_MUTATIONS must be true when LUMONOX_ENV=production "
            "and DASHBOARD_AUTH_ENABLED is true"
        )
...
def validate_event_plane_settings(settings: Settings) -> None:
    ...
    if settings.event_plane_mode not in {"duckdb_single_writer", "duckdb_log_shards"}:
        raise ValueError(
            "LUMONOX_EVENT_PLANE_MODE must be one of: duckdb_single_writer, duckdb_log_shards"
        )
    if settings.event_plane_mode == "duckdb_log_shards" and settings.event_store != "duckdb":
        raise ValueError(
            "LUMONOX_EVENT_PLANE_MODE=duckdb_log_shards requires LUMONOX_EVENT_STORE=duckdb"
        )
...
def normalize_database_url(database_url: str) -> str:
    ...
    if not raw_path:
        return normalized
    # Anchor relative SQLite paths to the same data root as DuckDB (LUMONOX_DATA_DIR / …).
    data_root = resolve_lumonox_data_root()
    if (
        raw_path.startswith("/./")
    ...
    # ``.lumonox/`` with DuckDB and the email outbox.
    _legacy_root_sqlite = frozenset({"lumonox.db", "lumonox_embedded.db"})
    data_root_resolved = data_root.resolve()
    if resolved.name in _legacy_root_sqlite and resolved.parent.resolve() == data_root_resolved:
        resolved = (data_root_resolved / ".lumonox" / resolved.name).resolve()
    normalized_path = str(resolved).replace("\\", "/")
    out = f"{parsed.scheme}:///{normalized_path}"
    # SQLite does not create parents; nested defaults live under `.lumonox/` like DuckDB.
...
def resolve_lumonox_data_root() -> Path:
    """Directory used to anchor relative ``LUMONOX_DUCKDB_PATH`` values.

    Precedence:

    1. ``LUMONOX_DATA_DIR`` (preferred)
    2. ``LUMONOX_PROJECT_ROOT`` (alias for the same semantics)
    ...
    """
    for key in ("LUMONOX_DATA_DIR", "LUMONOX_PROJECT_ROOT"):
...
def normalize_event_store_duckdb_path(raw: str, *, data_root: Path | None = None) -> str:
    """Resolve ``LUMONOX_DUCKDB_PATH`` to an absolute path independent of process cwd.

    Relative paths are joined under ``data_root`` (default: :func:`resolve_lumonox_data_root`).
    Absolute paths are expanded and resolved as-is.

    Raises:
        ValueError: if a relative path escapes ``data_root`` after resolution.
    """
    text = (raw or "").strip() or "./.lumonox/events.duckdb"
    root = (data_root or resolve_lumonox_data_root()).resolve()
    ...
    try:
        ...
        raise ValueError(
            f"LUMONOX_DUCKDB_PATH={raw!r} resolves outside data root {root}: {anchored}"
        ) from exc
    return str(anchored)


def normalize_event_plane_shards_path(raw: str, *, data_root: Path | None = None) -> str:
    """Resolve ``LUMONOX_EVENT_PLANE_SHARDS_PATH`` to an absolute path."""
    text = (raw or "").strip() or "./.lumonox/events-log"
    root = (data_root or resolve_lumonox_data_root()).resolve()
    candidate = Path(text).expanduser()
    if candidate.is_absolute():
        return str(candidate.resolve())
    ...
    try:
        ...
        raise ValueError(
            f"LUMONOX_EVENT_PLANE_SHARDS_PATH={raw!r} resolves outside data root {root}: {anchored}"
        ) from exc
    return str(anchored)


def normalize_event_plane_snapshots_path(raw: str, *, data_root: Path | None = None) -> str:
    """Resolve ``LUMONOX_EVENT_PLANE_SNAPSHOTS_PATH`` to an absolute path."""
    text = (raw or "").strip() or "./.lumonox/events-duckdb"
    root = (data_root or resolve_lumonox_data_root()).resolve()
    candidate = Path(text).expanduser()
    if candidate.is_absolute():
    ...
    try:
        ...
        raise ValueError(
            "LUMONOX_EVENT_PLANE_SNAPSHOTS_PATH="
...
def normalize_alert_email_file_outbox_dir(raw: str | None, *, data_root: Path | None = None) -> str:
    ...
    Relative values are anchored under ``data_root`` (default: :func:`resolve_lumonox_data_root`)
    so magic-link and alert ``file`` delivery lands beside other workspace data regardless of
    process ``cwd`` when starting uvicorn (monorepo uses repo ``.lumonox/emails``).
    """
    text = (raw or "").strip() or "./.lumonox/emails"
    root = (data_root or resolve_lumonox_data_root()).resolve()
    candidate = Path(text).expanduser()
    if candidate.is_absolute():
        return str(candidate.resolve())
...
def normalize_parquet_export_root(raw: str | None, *, data_root: Path | None = None) -> str:
    """Resolve ``LUMONOX_PARQUET_EXPORT_ROOT`` to an absolute path."""
    text = (raw or "").strip() or "./.lumonox/parquet/events"
    root = (data_root or resolve_lumonox_data_root()).resolve()
    candidate = Path(text).expanduser()
    if candidate.is_absolute():
        return str(candidate.resolve())
    ...
    try:
        ...
        raise ValueError(
            f"LUMONOX_PARQUET_EXPORT_ROOT={raw!r} resolves outside data root {root}: {anchored}"
        ) from exc
    return str(anchored)


def normalize_parquet_restore_root(raw: str | None, *, data_root: Path | None = None) -> str:
    """Resolve ``LUMONOX_PARQUET_OBJECT_STORAGE_RESTORE_ROOT`` to absolute path."""
    text = (raw or "").strip() or "./.lumonox/parquet/restore"
    root = (data_root or resolve_lumonox_data_root()).resolve()
    candidate = Path(text).expanduser()
    if candidate.is_absolute():
        return str(candidate.resolve())
    ...
    try:
        ...
        raise ValueError(
            "LUMONOX_PARQUET_OBJECT_STORAGE_RESTORE_ROOT="
            f"{raw!r} resolves outside data root {root}: {anchored}"
        ) from exc
    return str(anchored)
...
def _is_workspace_default_dev_sqlite_file(normalized_database_url: str) -> bool:
    ...
    return path is not None and path.name in {"lumonox.db", "lumonox_embedded.db"}
...
def get_settings() -> Settings:
    ...
    database_url = normalize_database_url(
        getenv("DATABASE_URL", "sqlite+aiosqlite:///./.lumonox/lumonox.db")
    )
    sqlite_cap_raw = getenv("LUMONOX_SQLITE_MAX_DB_FILE_MB")
    if sqlite_cap_raw is None or sqlite_cap_raw.strip() == "":
        sqlite_cap_raw = getenv("LUMONOX_EMBEDDED_MAX_DB_SIZE_MB")
    sqlite_max_db_file_mb = _env_optional_positive_int("LUMONOX_SQLITE_MAX_DB_FILE_MB")
    if sqlite_max_db_file_mb is None:
        sqlite_max_db_file_mb = _env_optional_positive_int("LUMONOX_EMBEDDED_MAX_DB_SIZE_MB")
    if (
    ...
    poll_raw = getenv("LUMONOX_RETENTION_PRESSURE_POLL_SECONDS")
    if poll_raw is None or poll_raw.strip() == "":
        ...
        retention_pressure_poll_seconds = _env_float(
            "LUMONOX_RETENTION_PRESSURE_POLL_SECONDS",
            0.0,
            minimum=0.0,
        )
    retention_pressure_min_interval_seconds = _env_float(
        "LUMONOX_RETENTION_PRESSURE_MIN_INTERVAL_SECONDS",
        15.0,
        minimum=5.0,
    )
    event_store = getenv("LUMONOX_EVENT_STORE", "duckdb").strip().lower() or "duckdb"
    if event_store not in {"duckdb", "sqlite"}:
        event_store = "duckdb"
    _data_root = resolve_lumonox_data_root()
    event_store_duckdb_path = normalize_event_store_duckdb_path(
        getenv("LUMONOX_DUCKDB_PATH", "./.lumonox/events.duckdb").strip()
        or "./.lumonox/events.duckdb",
        data_root=_data_root,
    )
    event_plane_mode = (
        getenv("LUMONOX_EVENT_PLANE_MODE", "duckdb_single_writer").strip().lower()
        or "duckdb_single_writer"
    )
    event_plane_shards_path = normalize_event_plane_shards_path(
        getenv("LUMONOX_EVENT_PLANE_SHARDS_PATH", "./.lumonox/events-log").strip()
        or "./.lumonox/events-log",
        data_root=_data_root,
    )
    event_plane_snapshots_path = normalize_event_plane_snapshots_path(
        getenv("LUMONOX_EVENT_PLANE_SNAPSHOTS_PATH", "./.lumonox/events-duckdb").strip()
        or "./.lumonox/events-duckdb",
        data_root=_data_root,
    )
    ...
    dashboard_oidc_cookie_name = (
        getenv("DASHBOARD_OIDC_COOKIE_NAME", "lumonox_oidc_state").strip() or "lumonox_oidc_state"
    )
    ...
    dashboard_realtime_bus_channel = (
        getenv("DASHBOARD_REALTIME_BUS_CHANNEL", "lumonox_dashboard_realtime").strip()
        or "lumonox_dashboard_realtime"
    )
    lumonox_env = getenv("LUMONOX_ENV", "development").strip().lower() or "development"
    settings = Settings(
        ...
        event_store_duckdb_single_writer_profile=_env_bool(
            "LUMONOX_DUCKDB_SINGLE_WRITER_PROFILE",
            False,
        ),
        ...
        parquet_export_enabled=_env_bool("LUMONOX_PARQUET_EXPORT_ENABLED", False),
        parquet_export_root=normalize_parquet_export_root(
            getenv("LUMONOX_PARQUET_EXPORT_ROOT", "").strip() or None,
            data_root=_data_root,
        ),
        ...
        parquet_query_enabled=_env_bool("LUMONOX_PARQUET_QUERY_ENABLED", False),
        parquet_hot_window_hours=_env_int(
            "LUMONOX_PARQUET_HOT_WINDOW_HOURS",
            24,
            minimum=1,
        ),
        parquet_lifecycle_enabled=_env_bool("LUMONOX_PARQUET_LIFECYCLE_ENABLED", False),
        ...
        parquet_lifecycle_dry_run=_env_bool("LUMONOX_PARQUET_LIFECYCLE_DRY_RUN", False),
        ...
        parquet_object_storage_enabled=_env_bool(
            "LUMONOX_PARQUET_OBJECT_STORAGE_ENABLED",
            False,
        ),
        parquet_object_storage_uri=getenv("LUMONOX_PARQUET_OBJECT_STORAGE_URI", "").strip() or None,
        ...
        parquet_object_storage_endpoint_url=getenv(
            "LUMONOX_PARQUET_OBJECT_STORAGE_ENDPOINT_URL",
            "",
        ).strip()
        or None,
        parquet_object_storage_region=getenv("LUMONOX_PARQUET_OBJECT_STORAGE_REGION", "").strip()
        or None,
        ...
        parquet_object_storage_restore_root=normalize_parquet_restore_root(
            getenv("LUMONOX_PARQUET_OBJECT_STORAGE_RESTORE_ROOT", "").strip() or None,
            data_root=_data_root,
        ),
        ...
        ingest_drop_lumonox_traffic_from_db=_env_bool(
            "INGEST_DROP_LUMONOX_TRAFFIC_FROM_DB",
            True,
        ),
        ...
        dashboard_auth_session_cookie_name=(
            getenv("DASHBOARD_AUTH_SESSION_COOKIE_NAME", "lumonox_dashboard_session").strip()
            or "lumonox_dashboard_session"
        ),
        ...
        dashboard_ws_live_tick_seconds=_env_float(
            "LUMONOX_DASHBOARD_WS_LIVE_TICK_SECONDS",
            4.0,
            minimum=0.0,
        ),
