...
class EmailAlertSender:
    ...
    async def send(self, signal: AlertSignal) -> AlertDeliveryResult:
        ...
        if provider == "resend":
            ...
            payload = {
                "from": self.from_email or "lumonox@localhost",
                "to": [destination],
                "subject": f"[Lumonox] {signal.alert_type} for project {signal.project_id}",
    ...
    async def _send_file(self, destination: str, signal: AlertSignal) -> AlertDeliveryResult:
        ...
        outbox_dir = Path(self.file_outbox_dir or "./.lumonox/emails").resolve()
        try:
...
def _format_alert_text(signal: AlertSignal) -> str:
    if signal.alert_type == "dashboard_magic_link":
        ...
        return (
            "Lumonox dashboard sign-in link\n"
            f"email: {email}\n"
    ...
    return (
        f"Lumonox alert: {signal.alert_type}\n"
...
def _build_email_message(
    ...
    msg = EmailMessage()
    msg["From"] = (from_email or "lumonox@localhost").strip() or "lumonox@localhost"
    msg["To"] = to_email
    msg["Subject"] = f"[Lumonox] {signal.alert_type} for project {signal.project_id}"
    msg["Date"] = formatdate(localtime=False)
    msg["Message-Id"] = make_msgid(domain="lumonox.local")
