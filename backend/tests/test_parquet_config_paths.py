...
def test_normalize_parquet_export_root_relative_is_under_data_root(tmp_path: Path) -> None:
    root = tmp_path / "repo"
    root.mkdir()
    resolved = normalize_parquet_export_root("./.lumonox/parquet/events", data_root=root)
    path = Path(resolved)
...
def test_normalize_parquet_export_root_rejects_path_escape(tmp_path: Path) -> None:
    root = tmp_path / "repo"
    root.mkdir()
    with pytest.raises(ValueError, match="LUMONOX_PARQUET_EXPORT_ROOT"):
...
def test_normalize_parquet_restore_root_rejects_path_escape(tmp_path: Path) -> None:
    root = tmp_path / "repo"
    root.mkdir()
    with pytest.raises(ValueError, match="LUMONOX_PARQUET_OBJECT_STORAGE_RESTORE_ROOT"):
