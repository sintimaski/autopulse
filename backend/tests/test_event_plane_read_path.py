...
from lumonox_backend.services.event_plane_read_path import (
...
def _settings(*, mode: str) -> Settings:
    return Settings(
        ...
        event_plane_snapshots_path="./.lumonox/events-duckdb",
    )
...
def test_cutover_selector_uses_legacy_store_when_toggle_disabled(monkeypatch) -> None:
    legacy = object()
    monkeypatch.setattr(
        "lumonox_backend.services.event_plane_read_path.get_duckdb_event_store",
        lambda: legacy,
    )
...
def test_cutover_selector_uses_snapshot_store_when_enabled_and_snapshot_present(
    ...
    monkeypatch.setattr(
        "lumonox_backend.services.event_plane_read_path.resolve_current_snapshot_duckdb_path",
        lambda _: Path("/tmp/snapshot/events.duckdb"),
    )
    monkeypatch.setattr(
        "lumonox_backend.services.event_plane_read_path.get_duckdb_read_store_for_path",
        lambda _: snapshot,
    )
