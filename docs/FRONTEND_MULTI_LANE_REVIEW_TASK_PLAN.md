...
| FE-06 | **Partial** | `settingsContentTypes.ts` + settings sections under `dashboardPages/` (retention, metrics, diagnostics, event-plane, exclude traffic, alert delivery, active project, orgs/members, API keys, appearance) + **`useSettingsOrganizationsMembers`** for org/member list state. `SettingsContent.tsx` still owns diagnostics/metrics/event-plane/alert test effects; further hook splits still open. |
...
| FE-09 | **Partial** | Guard unit tests + Playwright **`settings-smoke`**, **`alerts-smoke`**, **`logs-smoke`**, **`query-explorer-smoke`**, **`traces-smoke`** (`authDevMagicLink.ts`). Further interaction coverage still open. |
