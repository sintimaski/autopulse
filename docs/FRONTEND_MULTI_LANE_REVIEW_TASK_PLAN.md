...
| FE-06 | **Partial** | `settingsContentTypes.ts` + settings sections under `dashboardPages/` (retention, metrics, diagnostics, event-plane, exclude traffic, alert delivery, active project, orgs/members, API keys, appearance) + **`useSettingsOrganizationsMembers`**, **`useSettingsDiagnosticsPanels`** / **`settingsMetricStatusClass`**, **`useSettingsAlertDelivery`**, **`useSettingsApiKeyBulk`** (+ **`settingsApiKeyBulk`** helpers), **`useSettingsEventPlaneCutoverSave`**. `SettingsContent.tsx` still owns retention save, org invite POST, active-project switch, exclude-traffic toggle wiring; further splits still open. |
