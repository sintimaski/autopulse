...
| FE-09 | **Partial** | Guard unit tests + Playwright **`settings-smoke`**, **`alerts-smoke`**, **`logs-smoke`**, **`requests-smoke`**, **`query-explorer-smoke`**, **`traces-smoke`** (`authDevMagicLink.ts`). Further interaction coverage still open. |
