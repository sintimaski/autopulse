...
| FE-06 | **Partial** | `settingsContentTypes.ts`, appearance/retention/internal-metrics/system-diagnostics/event-plane/exclude-traffic/`SettingsAlertDeliverySection` under `dashboardPages/`. Orgs/members + API key lifecycle sections still inline in `SettingsContent.tsx`. |
