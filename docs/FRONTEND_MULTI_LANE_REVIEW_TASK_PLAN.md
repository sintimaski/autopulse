...
| FE-08 | **Partial** | **`dashboardSessionFetch.ts`** wraps **`fetchWithTimeout`** + **`buildApiUrl`** for session-scoped dashboard calls from **`DashboardDataContext`** and **`useSettingsDiagnosticsPanels`** (internal metrics, system diagnostics, event-plane cutover GETs; plus bootstrap, batch query, settings mutations, api-keys, validate, logout, diagnosis prefetch). Further consolidation (e.g. shared error parsing) still open. |
