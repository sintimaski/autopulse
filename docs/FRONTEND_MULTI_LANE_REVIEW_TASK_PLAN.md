...
| FE-08 | **Partial** | **`dashboardSessionFetch.ts`** wraps **`fetchWithTimeout`** + **`buildApiUrl`** for session-scoped dashboard calls from **`DashboardDataContext`**, **`useSettingsDiagnosticsPanels`** (diagnostics GETs), and **`useSettingsEventPlaneCutoverSave`** (event-plane cutover PUT); plus bootstrap, batch query, settings mutations, api-keys, validate, logout, diagnosis prefetch. Further consolidation (e.g. shared error parsing) still open. |
