...
logger = logging.getLogger("lumonox.synthetic_test_app")
...
def create_app(
    *,
    ...
    app = FastAPI(title="Lumonox Synthetic Test App", version="0.1.0")
    ...
    if enable_monitor:
        ...
        cfg = getattr(app.state, "_lumonox_config", None)
        if cfg is None:
            logger.warning(
                "synthetic_test_app: Lumonox monitor did not attach configuration (unexpected)."
            )
        elif not (cfg.api_key and (cfg.ingest_url or "").strip()):
            logger.warning(
                "synthetic_test_app: Lumonox is not sending events "
            ...
            logger.info(
                "synthetic_test_app: Lumonox active (remote), service=%s environment=%s",
                cfg.service_name,
                cfg.environment,
            )
