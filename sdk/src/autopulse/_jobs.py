...
def capture_background_job(
    ...
    *,
    ...
    Requires ``monitor()`` / ``autopulse()`` to have been applied on ``app`` so
    ``app.state._autopulse_dispatcher`` exists. Never raises to the caller.
    """
    try:
        dispatcher = getattr(app.state, "_autopulse_dispatcher", None)
        config = getattr(app.state, "_autopulse_config", None)
