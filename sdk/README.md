...
**`lumonox-sdk[stack]` on PyPI:** publish **`lumonox`** first (or same release train) so the extra can resolve the pinned **`lumonox`** floor (see `sdk/pyproject.toml` **`[project.optional-dependencies]`**).
