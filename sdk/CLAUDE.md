...
- Framework adapters (`lumonox.fastapi`, `lumonox.django`) are thin glue over the shared `lumonox.core.*` send path — never fork the dispatcher / scrubbing / event-shape / config-builder logic per adapter. FastAPI is primary; Django ships behind the `[django]` extra. See `sdk/docs/adapters.md`.
