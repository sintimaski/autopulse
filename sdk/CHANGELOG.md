...
- **Synthetic Django ASGI fixture.** ``sdk/src/lumonox/fixtures/synthetic_django_app.py`` mirrors ``synthetic_test_app.py`` (healthy route, parameterized route, view that raises). Exercised by ``sdk/tests/test_django_middleware.py`` (3 cases) through ``httpx.ASGITransport``. ``create_monitored_asgi_app()`` is a uvicorn ``--factory`` entry point that wires ``monitor()`` + ``wrap_asgi`` from ``LUMONOX_*`` env vars (kept a factory so importing the module stays side-effect-free for the unit tests).
- **Synthetic Django stack scripts.** ``scripts/run_synthetic_django_stack.sh`` runs the local integration stack with the Django fixture as the :8001 sample app — a thin wrapper that points the synthetic-app slot at the Django factory and delegates to ``run_synthetic_stack.sh`` (inherits the DuckDB-first backend defaults). ``scripts/examples/synthetic_django_load_demo.sh`` is a curl-loop traffic driver for the fixture's 3 routes.
