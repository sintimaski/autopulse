#!/usr/bin/env bash
# Build ``lumonox-sdk`` + ``lumonox`` wheels for offline / pre-PyPI installs.
...
mkdir -p "$OUT"
cd "$ROOT"
uv build --package lumonox --wheel -o "$OUT"
...
echo "  pip install \"$OUT\"/lumonox-*.whl \"$OUT\"/lumonox_sdk-*.whl"
echo "Parquet object storage with s3:// also needs boto3 (optional extra on the API wheel):"
echo "  pip install \"$OUT\"/lumonox-*.whl[parquet-s3] \"$OUT\"/lumonox_sdk-*.whl"
