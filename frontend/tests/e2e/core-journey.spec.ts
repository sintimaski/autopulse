...
async function signInViaDevMagicLink(page: Page) {
  ...
  await page.goto(`/autopulse/ui/auth/magic-link?token=${encodeURIComponent(token)}`);
  await page.waitForURL(/\/autopulse\/ui\/(dashboard|onboarding)/, { timeout: 15_000 });
}

test("sign-in, overview, and diagnosis load", async ({ page }) => {
  await page.goto("/autopulse/ui/dashboard");
  ...
  await page.goto("/autopulse/ui/diagnosis");
  await expect(page).toHaveURL(/\/autopulse\/ui\/diagnosis/);
  await expect(page.getByRole("link", { name: "Errors & Diagnosis" })).toBeVisible();
});
