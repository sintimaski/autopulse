      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500">
            ...
            stay on{" "}
            <Link href="/dashboard" className="font-medium text-sky-700 underline-offset-2 hover:underline">
              Dashboard
            </Link>{" "}
            and{" "}
            ...
            <li className="flex items-start justify-between gap-3">
              ...
              <span
                className={`rounded-full px-2 py-0.5 text-xs font-semibold ${
                  d.operationalSignals.errorSpikeCandidate
                    ? "bg-rose-500/15 text-rose-800"
                    : "bg-emerald-500/15 text-emerald-800"
                }`}
              >
                {d.operationalSignals.errorSpikeCandidate ? "Likely trigger" : "Within threshold"}
              </span>
            </li>
            <li className="flex items-start justify-between gap-3">
              ...
              <span
                className={`rounded-full px-2 py-0.5 text-xs font-semibold ${
                  d.operationalSignals.outageCandidate
                    ? "bg-rose-500/15 text-rose-800"
                    : "bg-emerald-500/15 text-emerald-800"
                }`}
              >
                {d.operationalSignals.outageCandidate ? "Likely trigger" : "No outage signal"}
              </span>
            </li>
          </ul>
          <p className="mt-3 text-xs text-slate-500">
            Based on current window: {overview.request_count} requests,{" "}
            {d.operationalSignals.successfulRequests} successful, {(overview.error_rate * 100).toFixed(1)}%
            error rate.
          </p>
        </div>
        <div className="rounded-xl border border-slate-200/90 bg-slate-50/50 p-4">
          <h3 className="text-xs font-semibold uppercase tracking-wide text-slate-600">Runbook shortcuts</h3>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            <button
              ...
              className="rounded-lg border border-slate-300 bg-white px-3 py-1.5 text-xs font-medium text-slate-800 shadow-sm transition hover:bg-slate-50"
            >
              Copy alerts-once
            </button>
            <button
            ...
            <button
              ...
              className="rounded-lg border border-sky-300 bg-sky-50 px-3 py-1.5 text-xs font-medium text-sky-900 shadow-sm transition hover:bg-sky-100"
            >
          ...
          {d.runbookMessage ? (
            <p className="mt-2 text-xs font-medium text-emerald-800" role="status" aria-live="polite">
              {d.runbookMessage}
            </p>
          ) : null}
        </div>
