...
export function SettingsContent() {
  ...
  // Operator-tab sections are all owner/admin gated. When the session role can
  // reach none of them, hide the Operator tab entirely instead of showing an
  // empty shell.
  const hasOperatorAccess =
    canViewInternalMetrics || canViewSystemDiagnostics || canManageEventPlaneCutover;
  ...
  const [tab, setTab] = useState<SettingsTabId>("project");
  // Guard against landing on the Operator tab after a role change removes access.
  const activeTab: SettingsTabId = tab === "operator" && !hasOperatorAccess ? "project" : tab;

  return (
    <div className="flex flex-col gap-4">
      <Tabs
        active={activeTab}
        onChange={(value) => setTab(value as SettingsTabId)}
        tabs={[
          { value: "project", label: "Project", icon: Boxes },
          ...(hasOperatorAccess
            ? [{ value: "operator" as const, label: "Operator", icon: Wrench }]
            : []),
        ]}
      />

      <div className="grid items-start gap-4 lg:grid-cols-[14rem_minmax(0,1fr)]">
        <SettingsSectionNav activeTab={activeTab} />
            ...
            <>
              <div id={settingsSectionAnchorId("active-project")} className="scroll-mt-24">
                <SettingsActiveProjectSection
                  ...
                  onReloadProjects={orgMembers.reloadOrganizations}
                />
              </div>
              ...
              <div id={settingsSectionAnchorId("api-keys")} className="scroll-mt-24">
                <SettingsApiKeyLifecycleSection
                  ...
                  keyBulkConfirmCount={apiKeyBulk.keyBulkConfirmCount}
                  onCancelKeyBulkConfirm={apiKeyBulk.cancelKeyBulkConfirm}
              ...
              <div id={settingsSectionAnchorId("members")} className="scroll-mt-24">
                <SettingsOrganizationsMembersSection
                  ...
                  onReloadMembers={orgMembers.reloadMembers}
                  ...
                  memberBulkConfirmCount={orgMembers.memberBulkConfirmCount}
                  onCancelMemberBulkConfirm={orgMembers.cancelMemberBulkConfirm}
