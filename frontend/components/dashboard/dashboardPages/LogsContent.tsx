...
export function LogsContent() {
  ...
  return (
    <>
      ...
      <section className="rounded-2xl border border-slate-200/80 bg-white/95 p-5 shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
        <div className="rounded-xl border border-slate-200/90 bg-slate-50/70 p-4 dark:border-neutral-700 dark:bg-neutral-800/70">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <h3 className="text-sm font-semibold text-slate-800 dark:text-neutral-100">SQL prompt logs</h3>
            <label className="flex items-center gap-2 text-xs text-slate-600 dark:text-neutral-300">
              <input
                type="checkbox"
                checked={d.liveQueryEnabled}
                onChange={(event) => d.setLiveQueryEnabled(event.target.checked)}
              />
              Live updates
            </label>
          </div>
          <textarea
            value={d.sqlQueryText}
            onChange={(event) => d.setSqlQueryText(event.target.value)}
            className="mt-2 h-24 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-mono shadow-sm outline-none ring-sky-500/30 focus:ring-2 dark:border-neutral-700 dark:bg-neutral-900 dark:text-neutral-100 dark:ring-neutral-600/40 dark:focus:ring-neutral-500/50"
          />
          <div className="mt-2 flex flex-wrap items-center gap-2">
            <button
              type="button"
              onClick={() => void d.runLogQueryValidate(d.sqlQueryText)}
              className="rounded-lg border border-slate-200 bg-white px-2 py-1 text-xs font-medium text-slate-700 hover:bg-slate-50 dark:border-neutral-700 dark:bg-neutral-900 dark:text-neutral-200"
            >
              Validate
            </button>
            <button
              type="button"
              onClick={() => void d.runLogQueryExecute(d.sqlQueryText, { resetCursor: true })}
              className="rounded-lg border border-sky-300 bg-sky-50 px-2 py-1 text-xs font-medium text-sky-800 hover:bg-sky-100 dark:border-sky-800 dark:bg-sky-950/40 dark:text-sky-200"
            >
              Run query
            </button>
            <button
              type="button"
              onClick={() => void d.runLogQueryExecute(d.sqlQueryText)}
              disabled={!d.sqlQueryCursor || d.sqlQueryLoading}
            ...
            {d.sqlQueryValidation ? (
              <span className={`text-xs ${d.sqlQueryValidation.valid ? "text-emerald-700 dark:text-emerald-300" : "text-rose-700 dark:text-rose-300"}`}>
                {d.sqlQueryValidation.valid ? "Valid query" : d.sqlQueryValidation.error ?? "Invalid query"}
              </span>
            ) : null}
          </div>
          {d.sqlQueryResults ? (
            <div className="mt-3 max-h-56 overflow-auto rounded-lg border border-slate-200 dark:border-neutral-700">
              <table className="min-w-full text-left text-xs">
                <thead className="bg-slate-100 text-slate-600 dark:bg-neutral-800 dark:text-neutral-300">
                  <tr>
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-neutral-800">
                  {d.sqlQueryResults.items.map((item) => (
                    <tr key={item.id}>
                      <td className="px-2 py-1">{formatTimestamp(item.timestamp)}</td>
                      <td className="px-2 py-1">{item.method}</td>
                      <td className="max-w-[280px] truncate px-2 py-1 font-mono">{item.path}</td>
                      <td className="px-2 py-1">{item.status_code}</td>
                      <td className="px-2 py-1 tabular-nums">{item.latency_ms.toFixed(1)} ms</td>
                    </tr>
                  ))}
                </tbody>
              </table>
