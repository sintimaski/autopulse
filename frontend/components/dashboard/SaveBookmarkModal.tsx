...
type SaveBookmarkModalProps = {
  ...
  hashFragment: string;
  onSaved?: () => void;
  /** When true, owner/admin may save as a team-visible bookmark for this project. */
  shareToProjectEligible?: boolean;
};
...
function SaveBookmarkModalBody({
  ...
  hashFragment,
  onSaved,
  shareToProjectEligible = false,
}: SaveBookmarkModalBodyProps) {
  const [title, setTitle] = useState(defaultTitle);
  const [notes, setNotes] = useState("");
  const [shareToProject, setShareToProject] = useState(false);
  ...
  const submit = async () => {
    ...
    try {
      await createDashboardBookmark({
        ...
        visibility: shareToProjectEligible && shareToProject ? "project" : "private",
      });
      onSaved?.();
      onClose();
    } catch (e) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <p className="text-sm text-slate-600 dark:text-neutral-400">
        Saved for this project. Open anytime from{" "}
        <span className="font-medium text-slate-800 dark:text-neutral-200">Bookmarks</span> in the sidebar.
      </p>
      {shareToProjectEligible ? (
        <label className="flex cursor-pointer items-start gap-2 text-sm text-slate-700 dark:text-neutral-200">
          <input
            type="checkbox"
            checked={shareToProject}
            onChange={(e) => setShareToProject(e.target.checked)}
            className="mt-0.5 size-4 rounded border-slate-300 text-orange-600 focus:ring-orange-500/40 dark:border-neutral-600 dark:bg-neutral-900"
          />
          <span>
            ...
            <span className="mt-0.5 block text-xs font-normal text-slate-500 dark:text-neutral-400">
              All members of this project can open this bookmark (not editable by everyone).
            </span>
          </span>
        </label>
      ) : null}
    ...
    <DashboardDetailModal open={open} title="Save bookmark" onClose={onClose} size="md">
      {open ? (
        <SaveBookmarkModalBody
          key={`${defaultTitle}|${hashFragment}|${pathname}|${shareToProjectEligible ? "1" : "0"}`}
          onClose={onClose}
          ...
          onSaved={onSaved}
          shareToProjectEligible={shareToProjectEligible}
        />
      ) : null}
